...
<form class="form-inline" action="<?php echo $_SERVER['PHP_SELF'] ?>" method="POST">
  ...
  <button type="submit" class="btn btn-primary">Extract details</button>
    ...
    <input type="text" class="form-control" id="brand_id" name="brand_id" value="<?=isset($row['brand_id'])?$row['brand_id']:""?>">
  </div>
    <h4>Edit brand</h4><hr style="margin-top: 10px;">
  <div class="form-group">
    <label class="control-label col-sm-2" for="brand_name">Brand</label>
    <div class="col-sm-9"> 
      <input type="text" class="form-control" name="brand_name" id="brand_name" value="<?=isset($row['brand_name'])?$row['brand_name']:""?>">
    </div>
  </div>
