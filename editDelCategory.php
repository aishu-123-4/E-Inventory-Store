...
<form class="form-inline" action="<?php echo $_SERVER['PHP_SELF'] ?>" method="POST">
  ...
  <button type="submit" class="btn btn-primary">Extract details</button>
    ...
    <input type="text" class="form-control" id="category_id" name="category_id" value="<?=isset($row['category_id'])?$row['category_id']:""?>">
  </div>
    <h4>Edit Category</h4><hr style="margin-top: 10px;">
  <div class="form-group">
    <label class="control-label col-sm-2" for="category_name">Category</label>
    <div class="col-sm-9"> 
      <input type="text" class="form-control" name="category_name" id="category_name" value="<?=isset($row['category_name'])?$row['category_name']:""?>">
    </div>
  </div>
